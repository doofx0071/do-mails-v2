...
export class EmailThreadingService {
   ...
   */
  private extractReferences(message: EmailMessage): string[] {
   ...
   */
  private getParticipants(message: EmailMessage): string[] {
   ...
   */
  private messagesMatch(msg1: EmailMessage, msg2: EmailMessage): boolean {
  ...
  groupMessagesIntoThreads(messages: EmailMessage[]): EmailThread[] {
    ...
    for (const message of sortedMessages) {
      ...
      for (const thread of threads) {
        const threadMatches = thread.messages.some(threadMsg => 
          this.messagesMatch(message, threadMsg)
        )
      ...
      if (targetThread) {
        ...
        targetThread.lastMessageAt = new Date(Math.max(
          targetThread.lastMessageAt.getTime(),
  ...
  addMessageToThread(thread: EmailThread, message: EmailMessage): EmailThread {
    ...
    const belongsToThread = thread.messages.some(threadMsg => 
      this.messagesMatch(message, threadMsg)
    )
  ...
  mergeThreadsIfNeeded(thread1: EmailThread, thread2: EmailThread): EmailThread | null {
    ...
    const shouldMerge = thread1.messages.some(msg1 =>
      thread2.messages.some(msg2 => this.messagesMatch(msg1, msg2))
    )
    ...
    return {
      ...
      lastMessageAt: new Date(Math.max(
        thread1.lastMessageAt.getTime(),
        thread2.lastMessageAt.getTime()
      )),
