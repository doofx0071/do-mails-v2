...
class EmailProcessor {
    constructor(mailgunService, threadingService, config) {
    ...
    async processInboundWebhook(webhookData) {
        try {
            ...
            throw new types_1.EmailProcessingError(`Failed to process inbound webhook: ${error.message}`, 'WEBHOOK_PROCESSING_ERROR', { originalError: error, webhookData });
        }
    }
    /**
    ...
    async sendEmail(request) {
        try {
            ...
            throw new types_1.EmailProcessingError(`Failed to send email: ${error.message}`, 'EMAIL_SEND_ERROR', { originalError: error, request });
        }
    }
    /**
    ...
    validateEmailMessage(message) {
        if (!message.id || !message.messageId) {
            throw new types_1.EmailProcessingError('Message missing required ID fields', 'VALIDATION_ERROR');
        }
        if (!message.from || !this.validateEmail(message.from)) {
            throw new types_1.EmailProcessingError('Message missing valid from address', 'VALIDATION_ERROR');
        }
        if (!message.to || message.to.length === 0) {
            throw new types_1.EmailProcessingError('Message missing to addresses', 'VALIDATION_ERROR');
        }
        // Validate all email addresses
        const allAddresses = [message.from, ...message.to, ...message.cc, ...message.bcc];
        for (const addr of allAddresses) {
            if (!this.validateEmail(addr)) {
                throw new types_1.EmailProcessingError(`Invalid email address: ${addr}`, 'VALIDATION_ERROR');
            }
        }
        ...
        if (message.attachments) {
            ...
            if (totalSize > this.config.maxAttachmentSize) {
                throw new types_1.EmailProcessingError(`Total attachment size exceeds limit: ${totalSize} > ${this.config.maxAttachmentSize}`, 'ATTACHMENT_SIZE_ERROR');
            }
        }
    }
    /**
    ...
    validateSendEmailRequest(request) {
        if (!request.from || !this.validateEmail(request.from)) {
            throw new types_1.EmailProcessingError('Invalid from address', 'VALIDATION_ERROR');
        }
        if (!request.to || request.to.length === 0) {
            throw new types_1.EmailProcessingError('Missing to addresses', 'VALIDATION_ERROR');
        }
        ...
        if (!request.subject || request.subject.trim().length === 0) {
            throw new types_1.EmailProcessingError('Missing email subject', 'VALIDATION_ERROR');
        }
        if (!request.text && !request.html) {
            throw new types_1.EmailProcessingError('Email must have either text or HTML content', 'VALIDATION_ERROR');
        }
    }
    /**
     * Generate UUID
     ...
     */
    generatePreview(message, maxLength = 150) {
