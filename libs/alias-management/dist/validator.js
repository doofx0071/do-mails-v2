...
class AliasValidator {
    constructor(config = {}) {
        this.config = {
            ...
            reservedAliases: config.reservedAliases ?? types_1.RESERVED_ALIASES,
            blockedPatterns: config.blockedPatterns ?? types_1.BLOCKED_PATTERNS,
     ...
     */
    validateFormat(aliasName, result) {
     ...
     */
    validateLength(aliasName, result) {
     ...
     */
    validateCharacters(aliasName, result) {
     ...
     */
    validateReservedAliases(aliasName, result) {
     ...
     */
    validateBlockedPatterns(aliasName, result) {
        const lowerAlias = aliasName.toLowerCase();
     ...
     */
    validateProfanity(aliasName, result) {
        const lowerAlias = aliasName.toLowerCase();
        for (const word of types_1.PROFANITY_WORDS) {
     ...
     */
    validateStructure(aliasName, result) {
     ...
     */
    generateSimilarAliases(aliasName, existingAliases) {
        const suggestions = [];
     ...
     */
    findSimilarAliases(aliasName, existingAliases) {
     ...
     */
    generateSuggestions(input, existingAliases = []) {
        const suggestions = [];
     ...
     */
    generateRandomAliases(count, existingAliases) {
        const suggestions = [];
