...
export class DomainValidator {
   ...
   */
  private validateFormat(domain: string, result: DomainValidationResult): void {
   ...
   */
  private validateLength(domain: string, result: DomainValidationResult): void {
   ...
   */
  private validateCharacters(domain: string, result: DomainValidationResult): void {
   ...
   */
  private validateLabels(domain: string, result: DomainValidationResult): void {
   ...
   */
  private validateTLD(domain: string, result: DomainValidationResult): void {
    const labels = domain.split('.')
    ...
    const hasValidTLD = Object.values(TLD_PATTERNS).some(pattern => pattern.test(domain))
   ...
   */
  private validateReservedDomains(domain: string, result: DomainValidationResult): void {
   ...
   */
  private validateDomainLists(domain: string, result: DomainValidationResult): void {
    const lowerDomain = domain.toLowerCase()
    ...
    if (this.allowedDomains && this.allowedDomains.length > 0) {
      const isAllowed = this.allowedDomains.some(allowed => 
        lowerDomain === allowed.toLowerCase() || 
