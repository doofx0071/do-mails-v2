  ...
  try {
      ...
      return NextResponse.json(
        {
          configured: false,
          message:
            'Mailgun API not configured. Please set MAILGUN_API_KEY in your environment variables.',
          required_env_vars: [
            'MAILGUN_API_KEY',
    ...
    const debugInfo = {
      ...
      api_key_preview: process.env.MAILGUN_API_KEY
        ? `${process.env.MAILGUN_API_KEY.substring(0, 8)}...${process.env.MAILGUN_API_KEY.slice(-8)}`
        : 'not set',
      base_url: process.env.MAILGUN_BASE_URL || 'https://api.mailgun.net',
