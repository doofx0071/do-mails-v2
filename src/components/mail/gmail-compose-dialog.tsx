...
export function GmailComposeDialog({
    ...
    const fetchAddresses = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (addresses.length > 0 && !form.getValues('from_address') && !replyTo) {
  ...
  return (
    <div className={cn(
      "fixed z-50 bg-white border border-gray-300 shadow-2xl rounded-lg overflow-hidden transition-all duration-200 flex flex-col",
      isMaximized 
        ? "inset-4" 
        : isMinimized 
        ? "bottom-0 right-6 w-80 h-10" 
        : "bottom-0 right-6 w-96 h-[600px]"
    )}>
      {/* Header */}
      <div className="flex items-center justify-between bg-gray-50 border-b px-4 py-2 flex-shrink-0">
        ...
        <div className="flex flex-col h-full">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col h-full">
              <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center gap-2">
                        <FormLabel className="text-xs w-12">From</FormLabel>
                        <FormControl>
                          <Select onValueChange={field.onChange} value={field.value}>
                            <SelectTrigger className="h-8 text-sm border-0 border-b rounded-none focus:ring-0">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center gap-2">
                        <FormLabel className="text-xs w-12">To</FormLabel>
                        <FormControl>
                          <Input 
                ...
                {showCc && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <div className="flex items-center gap-2">
                          <FormLabel className="text-xs w-12">Cc</FormLabel>
                          <FormControl>
                            <Input 
                ...
                {showBcc && (
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <div className="flex items-center gap-2">
                          <FormLabel className="text-xs w-12">Bcc</FormLabel>
                          <FormControl>
                            <Input 
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <div className="flex items-center gap-2">
                        <FormLabel className="text-xs w-12">Subject</FormLabel>
                        <FormControl>
                          <Input 
                            placeholder="Subject" 
                            {...field}
                  ...
                  <div className="border rounded p-2 space-y-1">
                    ...
                    {attachments.map((file, index) => (
                      <div key={index} className="flex items-center justify-between text-xs bg-gray-50 rounded px-2 py-1">
                        ...
                        <span className="text-gray-500 mx-2">{Math.round(file.size / 1024)}KB</span>
                        <Button
                          type="button"
                          variant="ghost"
                          size="sm"
                    ...
                    <FormItem>
                      <FormControl>
                        <Textarea 
                          ...
                          className="resize-none border-0 focus-visible:ring-0 text-sm min-h-[120px] max-h-[200px]"
                          {...field}
                        />
                      </FormControl>
                      <FormMessage />
              ...
              {/* Footer */}
              <div className="border-t p-3 flex items-center justify-between flex-shrink-0 bg-white">
                <div className="flex items-center gap-1">
